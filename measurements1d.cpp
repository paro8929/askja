...
void WilsonLoop(double *eigenvalues)
{
  ...
  std::vector<double> myvector (eigenvalues, eigenvalues+NC);
...
void PlaqExpAlt(double *temporal, double *spatial) //probably needs to be divided by Nc
{
  ...
  *(spatial)=0;
  ...
  for (int i=0;i<xygrid.SIZE;i++)
	...
	for (int k=0;k<xygrid.gridsize.d;k++)
      ...
      for (int k=1;k<xygrid.gridsize.d;k++)
	*(temporal)-=tr2(Fij.M[k]);
      
      
      for (int j=1;j<xygrid.gridsize.d;j++)
	for (int k=j+1;k<xygrid.gridsize.d;k++)
	  *(spatial)-=tr2(Fij.M[j*xygrid.gridsize.d+k]);
    }
...
int main(int argc, char* argv[])
{
  ...
  if (!ABORT)
	...
	{	 
	  ...
	  if (VERBOSE)
	    printf("conf=%i Wx=%f Ut=%f Us=%f\n",conf,evs[0],Ut*BETA/vol,Us*BETA/vol);
	  //printf("conf=%i Wx=%f Plaq=%f\n",conf,evs[0],2*NC*em/AT);
	  Wssum+=evs[0];
	  Utsum+=Ut*BETA/vol;
	  Ussum+=Us*BETA/vol;
	  if (conf>SKIP)
	    rav << conf << "\t" << Wssum/(conf-SKIP+1) << "\t 0.\t" << (Utsum)/(conf-SKIP+1) << "\t 0.\t" << (Ussum)/(conf-SKIP+1) << "\t 0.\n";

	  for (int i=0;i<NC;i++)
	    {
	      int gotme=floor((evs[i+1]+M_PI)/width);
	      if (gotme>bins)
      ...
      for (int i=0;i<bins;i++)
	{
	  ...
	  Hist << dummydouble << "\t" << Whist[i]/(1.0*TAKEN-SKIP);
	  Hist << endl;
	  if (VERBOSE)
	    printf("%f %f\n",dummydouble,Whist[i]/(1.0*TAKEN-SKIP));
	}

      Hist.close();
      
      printf("Nt=%i Ns=%i BETA=%.2f <Wx>=%f+-%f Ut=%f+-%f Us=%f+-%f\n",mygrid.x[0],mygrid.x[1],BETA,Wssum,sqrt(dW),Utsum,sqrt(dUt),Ussum,sqrt(dUs));
