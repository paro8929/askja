...
int main(int argc, char* argv[])
{
  ...
  if (!ABORT)
	...
	P5sum[x0]=0;
      ...
      sprintf(buffer,"%s/W2-N%i-Nt%i-Ns%i-BETA%.2f-DT%.4f-U%.4f-CT%i.dat",outdir,NC,mygrid.x[0],mygrid.x[1],BETA,dt,UPDATE2*dt,CONFTAKE);
      ...
      for (int conf=SKIP;conf<TAKEN;conf++)//first pass
	{
	  ...
	  Wssum+=evsW[0];
	  Psum+=evs[0];
	  W2sum+=evsW[0]*evsW[0]+evs[0]*evs[0];
	  //Plaqsum+=em*BETA/vol;
	  Utsum+=Ut*BETA/vol;
	  Ussum+=Us*BETA/vol;
      ...
      for (int conf=SKIP;conf<TAKEN;conf++)//second pass
	{
	  ...
	  dW2+=(evsW[0]*evsW[0]+evs[0]*evs[0]-W2sum)*(evsW[0]*evsW[0]+evs[0]*evs[0]-W2sum);
	...
	dP5sum[x0]/=(TAKEN-SKIP)*(TAKEN-SKIP);
      ...
      W2 << "#NC" << "\t" << "BETA" << "\t" << "Ns" <<"\t"<< "W2" << "\t" << "dW2" << "\t" << "conf" << endl;
      W2 << NC    << "\t" << BETA << "\t" << xygrid.gridsize.x[1] << "\t" << W2sum << "\t" << sqrt(dW2) << "\t" << TAKEN-SKIP << endl;
